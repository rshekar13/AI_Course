...
def preprocess():
    ...
    col = pd.read_csv(r'model_deployment\col_names.csv').columns
    drop_col = pd.read_csv(r'model_deployment\drop_col.csv').columns
...
def predict():
    final_df = preprocess()
    model = joblib.load(r"model_deployment\xbg_best.pkl")
